...
export async function getUserOwnedProjects(userId: number) {
  ...
  const res = await fetch(`http://localhost:3000/users/${userId}/owned-projects`, {
    headers: {
      'Authorization': `Bearer ${token}`,
    },
  });
...
export async function getAllTasks(filters?: { status?: string; priority?: string; assigneeId?: number }) {
  const token = localStorage.getItem('token');
  if (!token) throw new Error('No token found');
